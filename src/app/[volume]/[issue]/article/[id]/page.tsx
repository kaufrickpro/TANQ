...
export default async function ArticlePage({ params }: PageProps) {
  const { volume, issue: issueParam, id } = await params;
  ...
  // Canonical redirection if the volume or issue params do not match the database values.
  const expectedVolumeStr = `volume${issue.volume}`;
  const expectedIssueStr = `issue${issue.number}`;

  if (volume !== expectedVolumeStr || issueParam !== expectedIssueStr) {
    redirect(`/${expectedVolumeStr}/${expectedIssueStr}/article/${article.id}`);
  }
